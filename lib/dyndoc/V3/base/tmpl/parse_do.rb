    ...
    def get_named_blck(b2,filter)
      ...
      if b2[0][0]==:named
        ...
        if vars[0,1]==">"
          ...
          if (ind=vars.rindex(/(#{CqlsDoc::FORMATS.join("|")})$/))
...
#puts name; p code
    ...
    def do_verb(tex,blck,filter)
      ...
      tex << CqlsDoc::VERB[@cfg[:format_doc]][:begin] << "\n"
      tex << code.strip << "\n"
      tex << CqlsDoc::VERB[@cfg[:format_doc]][:end] << "\n"
    end
...
#puts "do_eval2: code";p code
      ...
      code=CqlsDoc::Utils.dyndoc_raw_text(code) if mode.include? :test
...
#p parse(blck[1..-1],filter)
    ...
    def do_part(tex,blck,filter)
      ...
      file=File.join("part",file+"_part"+CqlsDoc::EXTS[out])
