...
#puts "FIN INIT TemplateManager"
      if $cfg_dyn and $cfg_dyn[:devel_mode] and $cfg_dyn[:devel_mode]==:test
        ...
        puts CqlsDoc.get_pathenv($curDyn[:rootDoc]).join(":")
      end
    end
    ...
    def init_path(input)
      ...
      CqlsDoc.setRootDoc(@cfg[:rootDoc],input.scan(/%%%path\((.*)\)/).flatten.map{|e| e.split(",")}.flatten.map{|e| e.strip.downcase}.join(":"))
...
#puts "Manager:init_keys";p @init_keys
    ...
    def init_model(input)
      ...
      if @cfg[:model_doc] and CqlsDoc.cfg_dir[:tmpl_path][@cfg[:format_doc]]
        @cfg[:model_doc]="Default" if @cfg[:model_doc].downcase=="default"
        model_doc=File.join("Model",CqlsDoc.cfg_dir[:tmpl_path][ @cfg[:format_doc]],@cfg[:model_doc])
        @pre_model << File.read(CqlsDoc.doc_filename(model_doc+"Pre"))
        @post_model = [File.read(CqlsDoc.doc_filename(model_doc+"Post"))]
#p @pre_model
#p @post_model
      end
...
#p @cfg[:pre_doc]
      if @cfg[:pre_doc]
        @cfg[:pre_doc].uniq.each{|t|
          @pre_doc << File.read(CqlsDoc.doc_filename(t))
        }
      end
#p @pre_doc
      ...
      if @cfg[:post_doc] 
        @cfg[:post_doc].uniq.each{|t|
          @post_doc << File.read(CqlsDoc.doc_filename(t))
        }
      end
...
=begin (later if other possibility fails!)
  def dyn_filter
    if $dyn and $dyn.tmpl.fmt and CqlsDoc::OutputFilter[$dyn.tmpl.fmt.to_sym]
      return filter
    else
      return self
    end
...
class Symbol
  ...
  def to_var
    if $dyn and (res=$dyn.tmpl.vars.extract(self.to_s))
      res
    else
      self
    end
