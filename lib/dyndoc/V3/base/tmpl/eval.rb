...
#p tmpl;p tags;p rest
      tmpl_name=tmpl
      tmpl_orig=CqlsDoc.doc_filename(tmpl.strip)
      tmpl=CqlsDoc.directory_tmpl? tmpl_orig
...
#puts "inside INPUT";p tmpl
        return "Dyndoc Error: "+tmpl_name+" is not reacheable!!! " unless tmpl
      	input=CqlsDoc.read_content_file(tmpl,{:doc => @doc})
      	@blocks[block]=input
...
#puts "lib";p lib
          tmpl_orig=CqlsDoc.doc_filename(lib.strip)
          tmpl=CqlsDoc.directory_tmpl? tmpl_orig
#puts "tmpl";p tmpl
          #TO REMOVE: init_dtag(tmpl)
=begin TO REMOVE
...
=end
      	  ...
      	  unless @libs.keys.include? tmpl
      	    input=CqlsDoc.read_content_file(tmpl)
      	    @libs[tmpl]=input
    ...
    def eval_LOAD_HELPERS(b,filter)
      ...
      if b.length>0
        b2=b.map{|e| filter.apply(e)}
#p b2
        pathenv=File.read(File.join(CqlsDoc.cfg_dir[:home],"helpers")).strip
        helpers = ""
...
#p filename
#p pathenv
            filename2=CqlsDoc.absolute_path(filename,pathenv)
#puts "names";p names
            if filename2
...
#p helpers
        CqlsDoc::V3::Helpers.module_eval(helpers)
      end 
    end
