...
#p alias_lines
    ...
    def PartTag.global_alias(aliases)
      ## global aliases
      @@alias=""
      @@sys_alias=File.join(CqlsDoc.cfg_dir[:sys],"alias")
      @@alias << File.read(@@sys_alias).chomp << "\n"  if File.exists? @@sys_alias
      @@home_alias=File.join(CqlsDoc.cfg_dir[:home],'alias')
      @@alias << File.read(@@home_alias).chomp << "\n"  if File.exists? @@home_alias
      if CqlsDoc.cfg_dir[:file]
        @@file_alias=File.join(CqlsDoc.cfg_dir[:file],'.dyn_alias') 
